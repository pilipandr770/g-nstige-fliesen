...
with app.app_context():
    m = Manufacturer.query.filter_by(slug='halcon').first()
    if not m:
        print('Manufacturer halcon not found')
        sys.exit(1)
    print(f"Manufacturer: {m.name} (id={m.id})")

    scraper = ContentScraperService()
    all_content = scraper.extract_all_content('halcon')
    ...
    for item in projects:
        if not item.get('title') or len(item.get('title',''))<2:
            print('  Skipped project without title')
            count_skipped += 1
            continue
        exists = ManufacturerContent.query.filter_by(manufacturer_id=m.id, source_url=item.get('source_url')).first()
        if exists:
    ...
    for item in blog_posts:
        if not item.get('title') or len(item.get('title',''))<2:
            print('  Skipped blog post without title')
            count_skipped += 1
            continue
        exists = ManufacturerContent.query.filter_by(manufacturer_id=m.id, source_url=item.get('source_url')).first()
        if exists:
