...
with app.app_context():
    m = Manufacturer.query.filter_by(slug='estudi-ceramico').first()
    if not m:
        print('Manufacturer estudi-ceramico not found')
        sys.exit(1)
    print(f"Manufacturer: {m.name} (id={m.id})")

    scraper = ContentScraperService()
    all_content = scraper.extract_all_content('estudi-ceramico')
    ...
    for item in projects:
        if not item.get('image_url'):
            print('  Skipped project without image:', item.get('title'))
            count_skipped += 1
            continue
        if not item.get('title') or len(item.get('title',''))<2:
            count_skipped += 1
        ...
        exists = ManufacturerContent.query.filter_by(manufacturer_id=m.id, source_url=item.get('source_url')).first()
        if exists:
            print('  Exists project, skip:', item.get('title'))
            count_skipped += 1
            continue
        content = ManufacturerContent(
            manufacturer_id=m.id,
    ...
    # blog posts
    posts = all_content.get('blog_posts', [])
    print(f"Processing blog posts: {len(posts)}")
    for item in posts:
        if not item.get('title') or len(item.get('title',''))<2:
            count_skipped += 1
            continue
        exists = ManufacturerContent.query.filter_by(manufacturer_id=m.id, source_url=item.get('url')).first()
        if exists:
            print('  Exists blog, skip:', item.get('title'))
            count_skipped += 1
            continue
        content = ManufacturerContent(
            manufacturer_id=m.id,
        ...
        db.session.add(content)
        count_added += 1
        print('  Added blog:', item.get('title'))

    m.last_sync = datetime.utcnow()
    db.session.commit()
    print('Done. Added:', count_added, 'Skipped:', count_skipped)
