...
# Try different Gazzini URLs
urls_to_check = [
    'https://www.gazzini.it/',
    'https://gazzini.it/',
    'https://www.gazzini.com/',
    'https://gazzini.com/',
]

print('🔍 Searching for Gazzini website...\n')
...
if not working_url:
    print('⚠️  No working Gazzini URL found')
    exit(1)
...
for text, href in list(nav_links.items())[:20]:
...
# Look for collections/products page
print('Collections/Products pages:')
for text, href in nav_links.items():
    if any(word in text.lower() or word in href.lower() for word in ['collezione', 'collection', 'prodotto', 'product', 'catalogo', 'catalog']):
        abs_href = urljoin(working_url, href)
        print(f'  {text:40} -> {abs_href}')
...
print('Blog/News:')
for text, href in nav_links.items():
    if any(word in text.lower() or word in href.lower() for word in ['blog', 'news', 'stampa', 'rassegna', 'articolo']):
        abs_href = urljoin(working_url, href)
        print(f'  {text:40} -> {abs_href}')
